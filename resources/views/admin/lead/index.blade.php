...
@section('page-title')
    <div class="row bg-title">
        <!-- .page title -->
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 bg-title-left">
...
$(document).ready(function() {
    ...
    $('#select-all').on('click', function() {
        ...
        $('input[type="checkbox"].dt-checkboxes', rows).prop('checked', this.checked);
    });
    ...
    $('table').on('change', 'input[type="checkbox"].dt-checkboxes', function() {
        ...
        // If all checkboxes are checked, set the "select all" checkbox checked
        if ($('input[type="checkbox"].dt-checkboxes:checked').length === $(
                'input[type="checkbox"].dt-checkboxes').length) {
    ...
    $('#select-all').on('click', function() {
        $('.select-row').prop('checked', this
            .checked); // Select only the checkboxes with class 'select-row'
    });
    ...
    $('#bulk-delete').on('click', function() {
        ...
        $('.select-row:checked').each(function() {
            ids.push($(this).data('id')); // Collect IDs of checked rows
        });

        if (ids.length > 0) {
            if (confirm("Are you sure you want to delete selected records?")) {
                $.ajax({
                    url: "{{ route('admin.leads.bulk-delete') }}", // Ensure route is correct
                    type: 'post',
                    data: {
                        ids: ids,
                        _token: '{{ csrf_token() }}'
